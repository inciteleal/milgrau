...
def kfs_plot(
    ...
    sns.set_style(
        "darkgrid",
        {"grid.color": "0.6", "grid.linestyle": ":", "axes.facecolor": "gainsboro"},
    )
    fig = plt.figure()
    ...
    plt.suptitle(kfs_title, fontweight="bold")
    plt.subplots_adjust(top=0.9)

    plt.plot(
        np.multiply(aerosol_backscatter_smooth, optical_prop_scale),
        np.divide(alt.values.tolist(), altitude_scale),
        color=colorgraph,
        linestyle="-",
        linewidth=1.5,
        label=str(lamb) + " nm " + channelmode,
        zorder=1,
    )

    plt.xlabel("Backscatter Coefficient [Mm$^{-1}$sr$^{-1}$]", fontweight="bold")
    plt.ylabel("Height a.g.l. [km]", fontweight="bold")
    for label in ax1.get_xticklabels():
        label.set_fontweight(500)
    for label in ax1.get_yticklabels():
        label.set_fontweight(500)
    ax1.legend(loc="best", markerscale=1.5, handletextpad=0.2)
    ...
    plt.plot(
        np.multiply(aerosol_extinction_smooth, optical_prop_scale),
        np.divide(alt.values.tolist(), altitude_scale),
        color=colorgraph,
        linestyle="-",
        linewidth=1.5,
    ...
    plt.xlabel("Extinction Coefficient [Mm$^{-1}$]", fontweight="bold")
    plt.ylabel("Height a.g.l. [km]", fontweight="bold")
    for label in ax2.get_xticklabels():
        label.set_fontweight(500)
    for label in ax2.get_yticklabels():
        label.set_fontweight(500)
    ax2.legend(loc="best", markerscale=1.5, handletextpad=0.2)
...
def sr_plot(
    ...
    mean_SR = np.mean(
        scattering_ratio[
            int(altitude_scale * tropopause / float(dfdict["vert_res"][0])) : int(
                altitude_scale * top_altitude / float(dfdict["vert_res"][0])
            )
        ]
    )
    ...
    smoothfactor = 25
    SR = savgol_filter(
        scattering_ratio.values.tolist(), smoothfactor, 3
    )  # Applying Savitzky–Golay filter, a digital filter applied to data points for smoothing the graph
    ...
    plt.xlabel("Scattering ratio", fontweight="bold")
    plt.ylabel("Height a.g.l. [km]", fontweight="bold")
    for label in ax1.get_xticklabels():
        label.set_fontweight(500)
    for label in ax1.get_yticklabels():
    ...
    for label in ax1.get_yticklabels():
        label.set_fontweight(500)
    ax3.legend(loc="best", markerscale=1.5, handletextpad=0.2)
    ...
    ax3.hlines(
        ...
        colors="darkblue",
        linestyles="dashed",
        label="Tropopause alttude",
    )
    ax3.axis(xmin=0, xmax=4)
    ax3.axis(ymin=altitude_min_02, ymax=altitude_max_02)
    ax3.text(1.2, 14, r"Mean SR = " + str(round(mean_SR, 2)))
    ax3.text(
        1.2,
        12.6,
        r"Savitzky–Golay: "
        + str(int(smoothfactor * float(dfdict["vert_res"][0])))
        + "m",
    )
...
def ggplidar(
    ppsignal_an, ppsignal_pc, glued_signal, altitude, gluing_central_idx, window_length
):
    ...
    rcsan = np.multiply(
        (ppsignal_an[:idx_max] * scale_factor), np.power(altitude[:idx_max], 2)
    )
    ...
    plt.plot(
        altitude[:idx_max] / 1000, rcspc, label="photo-counting", color="orangered"
    )
    plt.plot(
        altitude[:idx_max] / 1000, rcsglued, label="glued signal", color="darkmagenta"
    )
    ylims = plt.ylim()
    plt.vlines(
        [
            altitude[int(gluing_central_idx - window_length / 2)] / 1000,
            altitude[int(gluing_central_idx + window_length / 2)] / 1000,
        ],
        ylims[0] - 5e10,
        ylims[1] + 5e10,
        colors="k",
        linestyles="dashed",
        label="glueing region",
    )
    plt.ylim(ylims)

    plt.xlabel("Glued signal - AN + PC", fontweight="bold")
    plt.ylabel("Height a.g.l. [km]", fontweight="bold")
    for label in ax.get_xticklabels():
        label.set_fontweight(500)
    for label in ax.get_yticklabels():
        label.set_fontweight(500)
    ax.legend(loc="best", markerscale=1.5, handletextpad=0.2)
