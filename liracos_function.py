...
def meanrcs(
    ...
    (p1,) = ax.plot(
        rcsignalmean[str(lamb[0]) + channelmode].rolling(50, min_periods=1).mean().to_numpy(),
...
def ql(
    ...
    if qlchannelmode == "AN":
        if lamb == 355:
            colorfactor = rcsignalmean.mean() + 4 * rcsignalmean.std()
        elif lamb == 532:
            colorfactor = rcsignalmean.mean() + 3 * rcsignalmean.std()
        elif lamb == 1064:
            colorfactor = rcsignalmean.mean() + 1.25 * rcsignalmean.std()
    ...
    ax.set_ylim(
        [
            np.ceil(minscale_altql / float(dfdict["vert_res"][0])),
            np.ceil(maxscale_altql / float(dfdict["vert_res"][0])),
        ]
    )
    ...
    cbar = fig.colorbar(
        pcmesh,
        cax=cb_ax,
        ticks=np.linspace(rcsignalmean.min(), colorfactor, 5),
